...
function inv_rem(iy)
		...
		if not fget(mget(x,iy-i),3) then mset(x,iy-i,0) end
end
...
function can_deposit(dx)
		...
		--trace(y-inv_len()-1)
		if not can_turn(dx) and fget(mget(x+dx,y),2) and fget(mget(x,y-inv_len()-1),3) and mget(x,y-1)~=61 then return x+dx,y end
